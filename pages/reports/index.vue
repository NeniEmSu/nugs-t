      ...
      <div class="content">
        ...
        <div v-else class="collection">
          <nuxt-link
            ...
            class="report"
          >
            <div v-if="report._embedded['wp:featuredmedia']" class="lazy thumbnail">
              <img
                v-lazy="report._embedded['wp:featuredmedia'][0].media_details.sizes.full.source_url"
                :alt="report._embedded['wp:featuredmedia'][0].alt_text"
              />
              <Spinner1 class="spinner" />
            </div>
            <div v-if="report._embedded['wp:featuredmedia']" class="lazy medium">
              <img
                v-lazy="
                  report._embedded['wp:featuredmedia'][0].media_details.sizes.medium.source_url
                "
                :alt="report._embedded['wp:featuredmedia'][0].alt_text"
              />
              <Spinner1 class="spinner" />
            ...
            <div class="report-info">
              <div class="date">
                <span>{{ shortTimestamp(report.date) }}</span>
              </div>
              <h6 v-html="report.title.rendered"></h6>
            </div>
          </nuxt-link>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      heading: 'Reports',
      supHeading: 'Monthly, Quarterly, and More Reports.',
      desc:
      ...
      title: 'Images from our top reports',
      subTitle:
        'The National union of Ghanaian students attends regular reports and acquires multiple images while doing that and would love you to see them.',
      reports: [],
      loading: false,
    }
  },
  ...
  computed: {
    reportsList() {
      return [...this.reports].filter(
        (report) => !report.categories.includes(parseInt(process.env.FEATURED_CATEGORY_ID))
      )
    },
  },
  created() {
